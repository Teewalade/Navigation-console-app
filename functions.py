...
def get_location_coordinates(place):  # Cynthia
    """
    ...
    google_maps_api_key = os.getenv("GOOGLE_MAPS_API_KEY_PLACES")  # API key
...
def log_coordinates(place, longitude, latitude):  # Cynthia
    """
...
def direction():  # Tobi
    ...
    try:  # The try statement ensures the code doesn't break
        ...
        if mode not in ("walking", "driving", "bus", "train"):  # Checks  if the user inputted the recognized mode
            ...
            if is_direction == "yes":
                if len(direction) > 0: 
                    print(1) # Check the number of html instruction in the dictionary
...
def discover_places_service():  # Aishat
    """
...
def push_navigation_services():  # Zione
    """
...
def more_navigation_service():  # Zione
    """
